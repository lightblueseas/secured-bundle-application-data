...
public class JdbcUrlBean
{
	...
	public static JdbcUrlBeanBuilder DEFAULT_POSTGRESQL_URL = JdbcUrlBean.builder()
		.protocol("jdbc:postgresql://").host("localhost").port(5432).build().toBuilder();

	/** The default builder for the mysql jdbc url. */
	public static JdbcUrlBeanBuilder DEFAULT_MYSQL_URL = JdbcUrlBean.builder()
		.protocol("jdbc:mysql://").host("localhost").port(3306).build().toBuilder();
	...
	{
		return newPostgresJdbcUrl(JdbcUrlBean.DEFAULT_POSTGRESQL_URL.database(database).build());
	}
	...
	{
		return newMysqlJdbcUrl(JdbcUrlBean.DEFAULT_MYSQL_URL.database(database).build());
	}
